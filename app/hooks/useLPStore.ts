...
const useLPStore = create((set: any, get: any) => ({
    ...
    swap: async (trtn: number, usdc: number, swapType: string, confirmed: boolean) => {
        ...
        if (swapType === "trtn") {
            ...
            slippage = Math.abs(
                1 - usdc_to_send / TOKEN_MULTIPLIER / usdc
            );
            ...
            slippage = Math.abs(
                1 - trtn_to_send / TOKEN_MULTIPLIER / trtn
            );
        }
    ...
    getAccountStats: async () => {
        const usdcBalance = await get().psdnState.program.provider.connection.getTokenAccountBalance(get().psdnState.walletUsdcAccount)
            .then((balance: { value: { uiAmount: any; }; }) => balance?.value?.uiAmount ? balance.value.uiAmount : 0).catch(() => 0);
        const shellBalance = await get().psdnState.program.provider.connection.getTokenAccountBalance(get().psdnState.walletShellAccount)
            .then((balance: { value: { uiAmount: any; }; }) => balance?.value?.uiAmount ? balance.value.uiAmount : 0).catch(() => 0);
        const trtnBalance = await get().psdnState.program.provider.connection.getTokenAccountBalance(get().psdnState.walletTrtnAccount)
            .then((balance: { value: { uiAmount: any; }; }) => balance?.value?.uiAmount ? balance.value.uiAmount : 0).catch(() => 0);
        set({accountStats: {
                usdcBalance,
                shellBalance,
